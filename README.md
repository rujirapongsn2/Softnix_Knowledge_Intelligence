...
The extraction is deliberately opt-in and does not change the document's searchable status if it fails. It is an information-extraction aid, not legal advice; reviewers must verify every extracted item against the source document. Scanned PDFs still require OCR before legal extraction can run.
