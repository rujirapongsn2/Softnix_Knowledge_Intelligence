...
The request creates an asynchronous `EXTRACT_LEGAL_METADATA` job. The result is stored with the document and includes reviewable fields such as document type/number, parties and roles, effective/execution/expiry dates, governing law, jurisdiction, obligations, rights, prohibitions, penalties, definitions, risk flags, confidence, and evidence quotes. Numbered provisions are captured in `articles` (มาตรา), while later notices or amendment announcements are captured in `amendments` (ประกาศแก้ไข), each with dates, changes, and an evidence quote when available.
...
The upload form includes a **Document type** choice. Select **Legal document**, **Regulation / policy**, or **Contract** to preserve that classification and automatically queue legal-schema extraction after the document has been extracted, chunked, embedded, and indexed. The document stays searchable if legal extraction later fails; the extraction job records the error for review. **General document** keeps the standard knowledge-processing flow. It is an information-extraction aid, not legal advice; reviewers must verify every extracted item against the source document. Scanned PDFs still require OCR before legal extraction can run.
