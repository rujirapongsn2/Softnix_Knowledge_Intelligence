...
def test_query_endpoint_regression_general_knowledge_base_is_unaffected():
    ...
    # Seed this query's evidence directly; unrelated queued jobs must not
    # determine whether a retrieval regression test has searchable content.
    with SessionLocal() as db:
        ...
        db.add(doc)
        db.flush()
        services.replace_document_chunks(db, doc, text)
        db.commit()
