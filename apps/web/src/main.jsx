...
function App() {
  ...
  const createMcpToken = async (payload, label = "MCP") => {
    ...
    await loadAccess(); notify(`${label} token created. Copy it now; it will not be shown again.`); return result;
  };
  const rotateMcpToken = async (tokenId, label = "MCP") => {
    const result = await api(`/v1/tokens/${tokenId}/rotate`, {method: "POST"});
    await loadAccess(); notify(`${label} token rotated. The previous key was revoked; copy the new key now.`); return result;
  };
  ...
  const sideNav = <SideNav header={<div className="brand-lockup"><img src="/logo-softnix.png" alt="Softnix"/><SideNavHeading superheading="SOFTNIX" heading="Knowledge Intelligence"/></div>} topContent={<Button label="New Knowledge Base" variant="primary" onClick={() => switchView("knowledge-bases")}/>} collapsible>
    ...
    <SideNavSection title="ADMINISTRATION" subtitle="Connect and monitor" className="side-nav-category"><SideNavItem label="MCP Tokens" isSelected={activeView === "mcp-tokens"} onClick={() => switchView("mcp-tokens")}/><SideNavItem label="Ingest API" isSelected={activeView === "ingest-tokens"} onClick={() => switchView("ingest-tokens")}/><SideNavItem label="Logging" isSelected={activeView === "logs"} onClick={() => switchView("logs")}/></SideNavSection>
  </SideNav>;
...
function Documents({selectedKb, documents, documentTotal, documentOffset, setDocumentOffset, documentSearch, setDocumentSearch, documentStatusFilter, setDocumentStatusFilter, documentTypeFilter, setDocumentTypeFilter, documentsLoading, hasCompletedDocuments, showDeletedDocuments, setShowDeletedDocuments, uploadFile, setUploadFile, uploadTitle, setUploadTitle, uploadDocumentType, setUploadDocumentType, documentTemplates, uploadTemplateId, setUploadTemplateId, uploadMetadata, setUploadMetadata, createDocumentTemplate, updateDocumentTemplate, deactivateDocumentTemplate, activateDocumentTemplate, uploadDocument, isUploading, openDocument, extractLegalMetadata, saveLegalMetadata, deleteLegalMetadata, saveDocumentMetadata, reprocessDocument, deleteDocument, restoreDocument, reindexEmbeddings, refreshDocuments, documentPreview, documentJobs, legalInstruments, resolveLegalRegistry, updateLegalInstrument, entities, relationships, addEntity, addRelationship, impact, analyzeImpact, syncGraphFromDocuments, refreshGraph, isLegalGraph, legalGraphView, setLegalGraphView, queueLegalGraphRebuild, legalRebuildStatus, reviewLegalRelationship, onClosePreview, onCreateKb, onSearch, onExplore}) {
  ...
  return <><PageHeading eyebrow="DOCUMENTS" title={`Build ${selectedKb.name}`} description="Manage source files, processing status, and legal metadata in one place." actions={<><Button ref={typeManagerTriggerRef} label="Manage document types" variant="secondary" onClick={() => setIsTypeDrawerOpen(true)}/><Button label={showDeletedDocuments ? "Hide deleted" : "Show deleted"} variant="ghost" onClick={() => { setDocumentOffset(0); setShowDeletedDocuments(value => !value); }}/><Button label="Reindex embeddings" variant="secondary" onClick={reindexEmbeddings}/><Button label="Refresh status" variant="ghost" onClick={refreshDocuments}/></>}/>
    ...
    {legalInstruments?.length > 0 && <div className="log-tabs" role="tablist"><button role="tab" aria-selected={libraryTab === "files"} className={libraryTab === "files" ? "selected" : ""} onClick={() => setLibraryTab("files")}>Files</button><button role="tab" aria-selected={libraryTab === "legal"} className={libraryTab === "legal" ? "selected" : ""} onClick={() => setLibraryTab("legal")}>Legal instruments</button></div>}
    {(libraryTab === "files" || !(legalInstruments?.length > 0)) && <section className="content-section"><div className="section-title"><div><h2>{showDeletedDocuments ? "All documents" : "Library"}</h2><p>{documentTotal ? `Showing ${pageStart}–${pageEnd} of ${documentTotal} document${documentTotal === 1 ? "" : "s"}` : "Your uploaded documents will appear here."}</p></div>{documents.some(document => ["queued", "extracting", "indexing"].includes(document.status) || ["queued", "running"].includes(document.processing_job_status)) && <span className="live-status" role="status">Updating automatically</span>}</div>
...
const legalDateLabel = instrument => {
  ...
  return instrument.effective_to ? `มีผล ${instrument.effective_from} – ${instrument.effective_to}` : `มีผล ${instrument.effective_from}`;
};
...
function LegalMapPanel({map, loading, onSelectInstrument, onOpenDocument, onOpenAdvanced, resolveLegalRegistry}) {
  ...
  const renderCard = instrument => {
    const provenance = [instrument.authority_level != null ? `Authority ${instrument.authority_level}` : null, instrument.status_reason, instrument.source_reference].filter(Boolean).join(" · ");
    return <div key={instrument.id} role="button" tabIndex={0} className={`legal-instrument-card ${instrument.id === currentInstrument?.id ? "is-current" : ""}`} onClick={() => onSelectInstrument(instrument.id)} onKeyDown={event => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); onSelectInstrument(instrument.id); } }}>
      <div className="legal-instrument-card-top"><span className="legal-class-chip">{legalClassLabel(instrument)}</span><span className={`legal-status ${instrument.status || "unknown"}`}><i aria-hidden="true"/>{legalStatusLabel(instrument.status)}</span><Badge label={reviewStatusLabel(instrument.review_status)} variant={reviewBadgeVariant(instrument.review_status)}/></div>
      <h3>{instrument.title}</h3>
      <p className="legal-instrument-filename">{instrument.filename || instrument.document_id}</p>
      {provenance && <p className="legal-instrument-authority">{provenance}{instrument.source_uri && " · "}{instrument.source_uri && <a className="legal-instrument-source-link" href={instrument.source_uri} target="_blank" rel="noreferrer" onClick={event => event.stopPropagation()} onKeyDown={event => event.stopPropagation()}>แหล่งที่มา ↗</a>}</p>}
      ...
      <div className="legal-instrument-card-footer"><span className="legal-instrument-open">เปิดโครงสร้างเอกสาร <b>→</b></span><button type="button" className="legal-instrument-open-doc" onClick={event => { event.stopPropagation(); openSourceDocument(instrument, event); }} onKeyDown={event => event.stopPropagation()}>เปิดเอกสารต้นฉบับ</button></div>
    </div>;
  };
  ...
  return <section className="legal-map-shell">
    <div className="legal-map-header">
      ...
      <div className="preview-actions"><Button label="Resolve statuses" size="sm" variant="secondary" onClick={resolveLegalRegistry}/><Button label="Advanced graph" size="sm" variant="secondary" onClick={onOpenAdvanced}/></div>
    </div>
    ...
    {!loading && currentInstrument && <section className="legal-current-panel"><div className="legal-current-kicker"><span className="eyebrow">CURRENT VERSION</span><span className="legal-status in_force"><i aria-hidden="true"/>มีผลใช้บังคับ</span><Badge label={reviewStatusLabel(currentInstrument.review_status)} variant={reviewBadgeVariant(currentInstrument.review_status)}/><button type="button" className="legal-instrument-open-doc" onClick={event => openSourceDocument(currentInstrument, event)}>เปิดเอกสารต้นฉบับ</button></div><button type="button" className="legal-current-card" onClick={() => onSelectInstrument(currentInstrument.id)}><div><p className="legal-current-kind">{legalClassLabel(currentInstrument)}</p><h3>{currentInstrument.title}</h3><p>{currentInstrument.filename || currentInstrument.document_id}</p><div className="legal-current-meta"><span><b>{legalDateLabel(currentInstrument)}</b></span><span>{currentInstrument.entity_count || 0} โหนด</span><span>{currentInstrument.relationship_count || 0} ความสัมพันธ์</span>{currentInstrument.authority_level != null && <span>Authority {currentInstrument.authority_level}</span>}</div>{currentInstrument.status_reason && <p className="legal-current-reason">{currentInstrument.status_reason}</p>}</div><span className="legal-current-action">ดูโครงสร้างมาตรา <b>→</b></span></button></section>}
...
const INGEST_ENDPOINTS = [
  {method: "GET", path: "/ingest/knowledge-bases", description: "List the Knowledge Base this token can write to (always 0 or 1 item)."},
  {method: "POST", path: "/ingest/knowledge-bases/{kb_id}/documents", description: "Upload a single document. Returns 202 with a document_id to poll."},
  {method: "POST", path: "/ingest/knowledge-bases/{kb_id}/documents/batch", description: "Upload up to 20 documents in one request; check results[] for per-file status."},
  {method: "GET", path: "/ingest/knowledge-bases/{kb_id}/documents", description: "List and paginate documents already queued or processed for this Knowledge Base."},
  {method: "GET", path: "/ingest/documents/{document_id}", description: "Poll a single document's processing status."},
  {method: "GET", path: "/ingest/documents/{document_id}/jobs", description: "List the processing job (retry) history for a document."},
];
...
function McpTokensView({selectedKb, knowledgeBases, tokens, auditLogs, loadAccess, createMcpToken, rotateMcpToken, changeTokenState}) {
  ...
  const create = async event => { event.preventDefault(); setIsLoading(true); setFormError(""); try { const result = await createMcpToken({name, allowed_knowledge_base_ids: selectedKbs, allowed_tools: tools, allowed_scopes: [], allowed_ingest_knowledge_base_id: null, expires_at: expiresAt ? new Date(expiresAt).toISOString() : null, requests_per_minute: Number(rpm), max_concurrent_requests: Number(concurrency), query_timeout_seconds: Number(timeout)}); revealSecret(result.token); setName(""); } catch (error) { setFormError(error.message || "สร้าง MCP token ไม่สำเร็จ"); } finally { setIsLoading(false); } };
...
function IngestTokensView({selectedKb, knowledgeBases, tokens, auditLogs, loadAccess, createMcpToken, rotateMcpToken, changeTokenState}) {
  ...
  const create = async event => { event.preventDefault(); setIsLoading(true); setFormError(""); try { const result = await createMcpToken({name, allowed_knowledge_base_ids: [], allowed_tools: [], allowed_scopes: [INGEST_SCOPE], allowed_ingest_knowledge_base_id: ingestKbId, expires_at: expiresAt ? new Date(expiresAt).toISOString() : null, requests_per_minute: Number(rpm), max_concurrent_requests: Number(concurrency), query_timeout_seconds: Number(timeout)}, "Ingest"); revealSecret(result.token); setName(""); } catch (error) { setFormError(error.message || "สร้าง Ingest token ไม่สำเร็จ"); } finally { setIsLoading(false); } };
  const rotate = async token => { if (!window.confirm(`Rotate key for ${token.name}? The current key will be revoked immediately.`)) return; setMutatingTokenId(token.id); setActionError(""); try { const result = await rotateMcpToken(token.id, "Ingest"); revealSecret(result.token); await copy(result.token, "token"); } catch (error) { setActionError(error.message || "หมุน token ไม่สำเร็จ"); } finally { setMutatingTokenId(""); } };
  const changeState = async (token, action) => { if (action === "revoke" && !window.confirm(`Revoke ${token.name}? This cannot be undone and connected systems will stop working.`)) return; if (action === "disable" && !window.confirm(`Disable ${token.name}? Connected systems will stop working until it is enabled again.`)) return; setMutatingTokenId(`${token.id}:${action}`); setActionError(""); try { await changeTokenState(token.id, action); } catch (error) { setActionError(error.message || "เปลี่ยนสถานะ token ไม่สำเร็จ"); } finally { setMutatingTokenId(""); } };
  ...
  const statusLabel = {active: "ใช้งาน", inactive: "ปิดใช้งาน", revoked: "เพิกถอนแล้ว"};
  return <><PageHeading eyebrow="INGEST API" title="Push documents from your own systems" description="Create a write-only token scoped to a single Knowledge Base, then copy a ready-to-run snippet for your DMS, ERP, or sync script." actions={<Button label="Refresh status" variant="ghost" isLoading={accessLoading} onClick={() => { refreshAccess(); loadOperations(); }}/>}/>{(accessLoadError || operationsError || copyError) && <p className="inline-error access-error" role="alert">{accessLoadError || operationsError || copyError}</p>}<section className="mcp-overview"><div className="mcp-status"><span className="status-dot"/><div><b>{operationsError ? "Unavailable" : operations?.ready?.status || "Checking system"}</b><span>{operations ? `${Object.keys(operations.ready.dependencies || {}).length} dependencies online` : operationsError || "Loading dependencies"}</span></div></div><div className="mcp-endpoint"><span>API base</span><code>{ingestApiBase}</code><button type="button" onClick={() => copy(ingestApiBase, "endpoint")}>Copy</button></div></section><section className="mcp-grid"><Card padding={4}><div className="card-heading"><div><p className="eyebrow">STEP 1</p><h2>Create a scoped token</h2></div><Badge label="Secret shown once" variant="warning"/></div><form className="form-stack" onSubmit={create}><TextInput label="Token name" value={name} onChange={setName} placeholder="e.g. dms-sync-job" isRequired/><div className="scope-section ingest-scope"><div className="scope-heading"><b>Write access</b><Badge label="Ingest API" variant="warning"/></div><p className="section-copy">อนุญาตให้ระบบภายนอกส่งเอกสารเข้า Knowledge Base ที่เลือกไว้ด้านล่างผ่าน Ingest API — token ใบนี้จะเพิ่มเอกสารเข้าคลังความรู้ได้ด้วยตัวเอง</p><div className="ingest-kb-picker"><p className="section-copy">เลือก Knowledge Base เดียวที่ token นี้เขียนได้ — เฉพาะ Knowledge Base ที่ "Activate" แล้วจะปรากฏในรายการนี้ (Knowledge Base ที่สร้างใหม่จะเป็นสถานะ Draft จนกว่าจะกด Activate)</p><div className="scope-options">{activeKnowledgeBases.length ? activeKnowledgeBases.map(kb => <label key={kb.id} className={`scope-option ${ingestKbId === kb.id ? "selected" : ""}`}><input type="radio" name="ingest-kb" checked={ingestKbId === kb.id} onChange={() => setIngestKbId(kb.id)}/><span>{kb.name}</span></label>) : <p className="section-copy">No active Knowledge Bases. Activate one from Knowledge Bases before creating a token.</p>}</div></div></div><details className="advanced-options"><summary>Advanced limits</summary><div className="limit-grid"><label>Expiry (optional)<input type="datetime-local" value={expiresAt} onChange={event => setExpiresAt(event.target.value)}/></label><label>Requests/min<input type="number" min="1" max="10000" value={rpm} onChange={event => setRpm(event.target.value)}/></label><label>Concurrent requests<input type="number" min="1" max="100" value={concurrency} onChange={event => setConcurrency(event.target.value)}/></label><label>Timeout (seconds)<input type="number" min="1" max="300" value={timeout} onChange={event => setTimeoutValue(event.target.value)}/></label></div></details>{formError && <p className="inline-error" role="alert">{formError}</p>}<Button label="Create Ingest token" type="submit" variant="primary" isLoading={isLoading} isDisabled={!name.trim() || !ingestKbId}/></form></Card><Card padding={4}><p className="eyebrow">STEP 2</p><h2>Push documents from your own system</h2><p className="section-copy">ให้ระบบภายนอก (DMS, ERP, สคริปต์ sync) ส่งเอกสารเข้า Knowledge Base ได้โดยตรง อ่านรายละเอียดทั้งหมดได้ที่ <code>docs/INGEST_API.md</code></p><p className="section-copy">อัปโหลดจะตอบ <code>202</code> ทันทีเพราะงานเข้าคิวประมวลผลแบบ asynchronous จากนั้นให้ poll สถานะเอกสารจนได้ <code>completed</code> หรือ <code>failed</code>{snippetLanguage === "curl" && secret ? "" : " ตัวอย่างนี้อ่าน token จากตัวแปรสภาพแวดล้อม SOFTNIX_INGEST_TOKEN"}</p><div className="ingest-language-tabs">{Object.entries(INGEST_SNIPPETS).map(([key, {label}]) => <button key={key} type="button" className={snippetLanguage === key ? "selected" : ""} onClick={() => setSnippetLanguage(key)}>{label}</button>)}</div><div className="code-panel"><div className="code-panel-top"><b>{INGEST_SNIPPETS[snippetLanguage].label}</b><button type="button" onClick={() => copy(ingestSnippet, "ingest snippet")}>{copied === "ingest snippet" ? "Copied" : "Copy snippet"}</button></div><pre className="ingest-preview">{ingestSnippet}</pre></div>{secret && <div className="token-reveal"><b>New token — copy it now</b><code>{secret}</code><div className="token-reveal-actions"><button type="button" onClick={() => copy(secret, "token")}>{copied === "token" ? "Copied" : "Copy token"}</button><button type="button" onClick={hideSecret}>Hide token</button></div></div>}</Card></section><section className="content-section"><div className="section-title"><div><p className="eyebrow">API REFERENCE</p><h2>Endpoints available to Ingest tokens</h2><p className="section-copy">Base URL <code>{ingestApiBase}</code> · every call needs <code>Authorization: Bearer &lt;token&gt;</code> · full field-level docs in <code>docs/INGEST_API.md</code></p></div></div><div className="ingest-endpoint-list">{INGEST_ENDPOINTS.map(endpoint => <article key={`${endpoint.method} ${endpoint.path}`} className="ingest-endpoint-row"><span className={`ingest-endpoint-method ingest-endpoint-method-${endpoint.method.toLowerCase()}`}>{endpoint.method}</span><div><code>{endpoint.path}</code><p>{endpoint.description}</p></div></article>)}</div></section><section className="content-section"><div className="section-title"><div><p className="eyebrow">TOKEN MANAGEMENT</p><h2>Ingest tokens</h2></div><span className="section-copy">Revoke a token immediately if a machine or credential is no longer trusted.</span></div><div className="token-filter-bar"><TextInput label="Find a token" value={tokenSearch} onChange={setTokenSearch} placeholder="Token name or prefix"/><DesignSystemSelect label="Status" value={tokenFilter} onChange={setTokenFilter} options={[{value: "all", label: "All statuses"}, ...Object.entries(statusLabel).map(([value, label]) => ({value, label}))]}/></div>{actionError && <p className="inline-error" role="alert">{actionError}</p>}{accessLoading && !ingestTokens.length ? <p className="section-copy" role="status">Loading tokens…</p> : visibleTokens.length ? <div className="token-list">{visibleTokens.map(token => { const busy = mutatingTokenId.startsWith(`${token.id}:`) || mutatingTokenId === token.id; return <article className="token-row" key={token.id}><div><b>{token.name}</b> <Badge label="Ingest" variant="warning"/><p>{token.token_prefix}… · writes to {kbNames[token.allowed_ingest_knowledge_base_id] || token.allowed_ingest_knowledge_base_id || "ไม่พบ Knowledge Base"}</p><small>{token.requests_per_minute}/min · {token.max_concurrent_requests} concurrent · {token.query_timeout_seconds}s timeout{token.expires_at ? ` · expires ${new Date(token.expires_at).toLocaleString()}` : ""}</small></div><StatusBadge status={token.status}/><div className="document-actions">{token.status !== "revoked" && <Button label="Rotate key" size="sm" variant="secondary" isLoading={busy && mutatingTokenId === token.id} isDisabled={Boolean(mutatingTokenId)} onClick={() => rotate(token)}/>} {token.status === "active" && <Button label="Disable" size="sm" variant="secondary" isLoading={busy && mutatingTokenId.endsWith(":disable")} isDisabled={Boolean(mutatingTokenId)} onClick={() => changeState(token, "disable")}/>} {token.status === "inactive" && <Button label="Enable" size="sm" variant="secondary" isLoading={busy && mutatingTokenId.endsWith(":enable")} isDisabled={Boolean(mutatingTokenId)} onClick={() => changeState(token, "enable")}/>} {token.status !== "revoked" && <Button label="Revoke" size="sm" variant="destructive" isLoading={busy && mutatingTokenId.endsWith(":revoke")} isDisabled={Boolean(mutatingTokenId)} onClick={() => changeState(token, "revoke")}/>}</div></article>; })}</div> : <EmptyState title={ingestTokens.length ? "No matching tokens" : "No Ingest tokens yet"} description={ingestTokens.length ? "Try another status or search term." : "Create a token above to let an external system push documents."}/>}</section><section className="content-section"><h2>Recent audit activity</h2>{auditLogs.length ? <div className="audit-list">{auditLogs.map(row => <div key={row.id}><span>{row.action}</span><small>{row.target_type || "system"} · {new Date(row.created_at).toLocaleString()}</small></div>)}</div> : <EmptyState isCompact title="No activity yet" description="Administrative actions will appear here."/>}</section></>;
}
...
function DocumentPreview({preview, jobs, templates, legalInstrument, onExtractLegal, onSaveLegal, onDeleteLegal, onSaveDocumentMetadata, onUpdateLegalInstrument, onClose}) {
  ...
  useEffect(() => { setEditingLegal(false); setLegalError(""); setLegalDraft(JSON.stringify(preview.legal_metadata || {articles: [], amendments: []}, null, 2)); setTab("content"); }, [preview.document_id, preview.legal_metadata]);
  ...
  const tabs = [["content", "Content"], ["metadata", "Metadata"], ["legal", "Legal"], ["activity", "Processing"]];
